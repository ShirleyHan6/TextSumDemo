...
from gensim_sum_ext.coreference_resolution import find_coreference_replace_pronominal


def summary_highlight(text, coref, ratio, omit_placeholders):
    if coref:
        coref_text = find_coreference_replace_pronominal(text)
    else:
        coref_text = text

    sum_text = summarize(text, coref_text, ratio=ratio, omit_placeholders=omit_placeholders)
